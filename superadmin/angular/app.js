angular
  ...
  .controller("subtopicADDCtrl", function($http, $q) {
    ...
    subtopic.fetchTopic = function(selectedCategory, type) {
      ...
      var req = {
        url: "http://localhost/padhlepappu/superadmin/API/getTopics.php",
        method: "POST",
	  ...
	  console.log(response);
        ...
        if (response.data.length == 0) {
          subtopic.topictext = "No data To Display";
        } else {
          subtopic.topictext = "Please Select";
        }
    ...
    subtopic.fetchsubTopic = function(selectedID, type, f) {
      ...
      var req = {
        url: "http://localhost/padhlepappu/superadmin/API/getSubTopic.php",
        method: "POST",
        data: {}
      };
      if (f == 1) {
        req.data.folderID = selectedID;
      } else {
      ...
      $http(req).then(function(response) {
        ...
        if (response.data.length == 0) {
          subtopic.subtopictext = "No data To Display";
        } else {
          subtopic.subtopictext = "Please Select";
        }
        defer.resolve(response.data);
      });
      return defer.promise;
    ...
    subtopic.fetchFolder = function(selectedID, type) {
      ...
      var req = {
        url: "http://localhost/padhlepappu/superadmin/API/getFolder.php",
        method: "POST",
      ...
      $http(req).then(function(response) {
        ...
        if (response.data.length == 0) {
          subtopic.foldertext = "No data To Display";
        } else {
          subtopic.foldertext = "Please Select";
        }
        defer.resolve(response.data);
      });
      return defer.promise;
